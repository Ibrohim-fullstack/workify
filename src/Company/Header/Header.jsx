  ...
  useEffect(() => {
    ...
    if (token && userInfo) {
      try {
        ...
        handleLogout(); // Ma'lumot buzilgan bo'lsa logout qilish xavfsizroq
      }
    }
  }, []);

  // TO'LIQ LOGOUT MANTIQI
  const handleLogout = () => {
    // 1. Storage'larni tozalash
    localStorage.clear();
    sessionStorage.clear();

    // 2. Local state'larni tozalash
    setUser(null);
    setIsOpen(false);
    setIsLogoutModalOpen(false);
    setShowUserMenu(false);

    // 3. Yo'naltirish (window.location.href tizimni to'liq reset qiladi)
          ...
          <div className="flex items-center gap-3 lg:gap-6">
            {user ? (
                  ...
                  {showUserMenu && (
                    <div className="absolute top-[55px] right-0 w-[220px] bg-white shadow-2xl rounded-2xl py-2 border border-gray-100 animate-in fade-in zoom-in duration-200 z-[60]">
                      <div className="px-4 py-3 border-b border-gray-50 mb-1">
                        <p className="text-[11px] uppercase tracking-wider text-gray-400 font-bold">Company</p>
                        <p className="text-[14px] font-bold truncate text-[#163D5C]">{user.company_name}</p>
                      </div>
                      <Link to="/my-company" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-[#343C44] font-semibold transition-colors">
              ...
              <div className="flex items-center gap-2 md:gap-3">
                <Link to="/signin" className="hidden sm:flex px-6 py-2.5 text-[16px] items-center justify-center bg-white border-2 border-[#163D5C] rounded-xl font-bold text-[#163D5C] hover:bg-[#163D5C] hover:text-white transition-all">
                  Sign in
                </Link>
                <Link to="/signup" className="flex px-6 py-2.5 text-[16px] items-center justify-center bg-[#163D5C] border-2 border-[#163D5C] rounded-xl font-bold text-white hover:bg-white hover:text-[#163D5C] transition-all shadow-md">
                  Join Now
                </Link>
              </div>
            )}
            ...
            <div className="hidden lg:block relative ml-2">
              <button onClick={() => setShowLang(!showLang)} className="flex items-center gap-1 font-bold text-[#343C44] hover:text-[#163D5C] transition-colors">
                {lang}
        ...
        <div className={`absolute right-0 top-0 h-full w-[300px] bg-white shadow-2xl transition-transform duration-300 flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
          ...
          <div className="p-6 flex flex-col gap-6 flex-1 overflow-y-auto">
            {user && (
              <div className="flex items-center gap-3 p-4 bg-blue-50/50 rounded-2xl">
                ...
                <div className="overflow-hidden">
                  ...
                  <p className="text-xs text-[#163D5C] font-semibold">Verified Company</p>
                </div>
              </div>
            )}
            ...
            <nav className="flex flex-col gap-2">
              ...
              {user && (
                <>
                  <MobileNavLink to="/dashboard" icon={<MdDashboard size={24} />} label="Dashboard" onClick={() => setIsOpen(false)} />
            ...
            <div className="mt-auto flex flex-col gap-3">
              {!user ? (
                <>
                  <Link to="/signin" onClick={() => setIsOpen(false)} className="w-full h-[54px] flex items-center justify-center border-2 border-[#163D5C] rounded-xl font-bold text-[#163D5C] active:scale-95 transition-all">
                    Sign in
                  </Link>
                  <Link to="/signup" onClick={() => setIsOpen(false)} className="w-full h-[54px] flex items-center justify-center bg-[#163D5C] rounded-xl font-bold text-white shadow-lg active:scale-95 transition-all">
                    Join Now
                  </Link>
                </>
              ) : (
